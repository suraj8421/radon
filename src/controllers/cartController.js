...
const updateCart = async function (req, res) {
    try {
        ...
        const findProduct = await productModel.findOne({ _id: cartId, isDeleted: false })
        ...
        for (let i = 0; i < findCart.items.length; i++) {
            if (findProduct._id.toString() == findCart.items[i].productId.toString()) {
                if (removeProduct == 1 && findCart.items[i].quantity > 1) {
                    ...
                    let updateCart = await cartModel.findOneAndUpdate({ _id: cartId }, { $inc: { totalPrice: -(findProduct.price) }, totalItems: findCart.items.length }, { new: true }).lean()
                    updateCart.items = findCart.items
                    return res.status(400).send({ status: true, message: "cart updated successfully", data: updateCart })
                } else {
                    ...
                    return res.status(400).send({ status: true, message: "removed successfully", data: updateCart })
                }
            }
        }
    } catch (err) {
...
const getCart = async function (req, res) {
    try {
        ...
        //     return res.status(404).send({ status: false, message: "cart is not Present" })
        // }
...
const deleteCart = async function (req, res) {
    try {
        ...
        console.log(cart);
        return res.status(204).send({ status: true, msg: "cart is Successfully Deleted", data: cart })
    }
